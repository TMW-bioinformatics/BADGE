...
echo -e "\e[103m\e[1mBADGE - Search for $type1 specific DMGs\e[0m"
echo -e "\e[34m#######################################################################\e[0m"
echo
echo
echo -e "\e[34m#######################################################################\e[0m"
...
if grep -i "error" *.log
	then
	echo -e "\e[31m\e[1mError - makeblastdb command failed - Check input fasta files\e[0m" >&2
	break
else
	echo -e "\e[1mCreating BLAST databases finished\e[0m"
	echo
fi
...
if $protein_level
	...
	echo
	echo -e "\e[34m#######################################################################\e[0m"
	echo -e "\e[1mStep 1 of $numsteps:  BLASTP all $type2 orfs vs $type1 orfs - Get $type1 DMGs\e[0m"
	echo
	...
	echo
	echo -e "\e[34m#######################################################################\e[0m"
	echo -e "\e[1mStep 1 of $numsteps:  $mb_display all $type2 orfs vs $type1 orfs - Get $type1 DMGs\e[0m"
	echo
...
echo -e "Number of DMGs after orf filter: \e[32m\e[1m$num_DMGs_orf_filtered\e[0m."
...
if [[ -s "Step1_4_"$type1"_DMGs.fasta" ]]
	then	
		echo
		echo -e "\e[34m#######################################################################\e[0m"
		echo -e "\e[1mStep 2 of $numsteps:  $mb_display $type1 DMGs vs $type2 genomes - Genome filter $type1 DMGs\e[0m"
		echo
	else
		echo -e "\e[31m\e[1mError - Step 2 of $numsteps cant be performed - No input from Step 1\e[0m" >&2
		cd $BADGE_path			
		return 1
fi
...
echo -e "Number of DMGs after genome filter: \e[32m\e[1m$num_DMGs_genomes_filtered\e[0m."
...
if [[ ! -s "Step2_4_"$type1"_DMGs.fasta" ]]
	then
		echo -e "\e[31m\e[1mError - Step 3 of $numsteps cant be performed - No input from Step 2\e[0m" >&2
		cd $BADGE_path			
		return 1
	elif $protein_level
		then
		...
		echo
		echo -e "\e[34m#######################################################################\e[0m"
		echo -e "\e[1mStep 3 of $numsteps:  BLASTP $type1 DMGs orfs vs all single $type1 orfs - Occurrence filter $type1 DMGs\e[0m"
		echo
	...
	if [[ $num_blast_proc -gt $seq_num ]]
		...
		echo
		echo -e "\e[34m#######################################################################\e[0m"
		echo -e "\e[1mStep 3 of $numsteps:  $mb_display $type1 DMGs orfs vs all single $type1 orfs - Occurrence filter $type1 DMGs\e[0m"
		echo
...
#check if DMGs remain after min_DMG_occurrence filter - if not proceed prompt user for new minimum DMG occurrence or continue with next type-combination
	...
	if [[ ! -s "Step3_4_"$type1"_DMGs_tmp.identifier" ]]
		then
			echo -e "\e[31m\e[1mStep 4 of $numsteps cant be performed - No input from Step 3\e[0m" >&2
			echo
...
echo -e "Number of DMGs after occurrence filter: \e[32m\e[1m$num_DMGs_nblast\e[0m."
...
if $dc_filter
	...
	echo
	echo -e "\e[34m#######################################################################\e[0m"
	echo -e "\e[1mStep 4 of $numsteps:  DC-MEGABLAST $type1 DMGs vs $type2 genomes - DC filter $type1 DMGs\e[0m"
	echo
	...
	if [[ ! -s "Step4_4_"$type1"_DMGs.identifier" ]]
		then
			echo -e "\e[31m\e[1mError - Step 5 of $numsteps cant be performed - No input from Step 4\e[0m" >&2
			cd $BADGE_path			
			return 1
	fi
	...
	echo -e "Number of DMGs after dc_megablast filter: \e[32m\e[1m$num_DMGs_dc_megablast\e[0m."
	...
	echo
	echo -e "\e[34m#######################################################################\e[0m"
	echo -e "\e[1mStep 4 of $numsteps is skipped:  DC-filter not active\e[0m"
	echo
...
if $blastn_filter
	...
	echo
	echo -e "\e[34m#######################################################################\e[0m"
	echo -e "\e[1mStep 5 of $numsteps:  BLASTN $type1 DMGs vs $type2 genome sequences - BLASTN filter $type1 DMGs\e[0m"
	echo
	...
	echo -e "Number of DMGs after blastn filter: \e[32m\e[1m$num_DMGs_blastn_hits\e[0m."
...
##############################################################################################################################################################
	...
	echo
	echo -e "\e[34m#######################################################################\e[0m"
	echo -e "\e[1mStep 5 of $numsteps is skipped:  BLASTN filter not active\e[0m"
	echo
...
echo
echo -e "\e[34m#######################################################################\e[0m"	
echo -e "\e[1mStep 6 of $numsteps:  Create BADGE output\e[0m"
echo
